...
def send_distance(dist_cm):
    ...
    数据字节编码：实际距离乘以10后，按有符号int8补码发送。
    正数编码保持不变，例如：+2.8cm -> 28 -> 0x1C。
    负数使用标准补码，例如：-1.0cm -> -10 -> 0xF6，
    -2.8cm -> -28 -> 0xE4。
    """
    # round用于避免4.5×10因为浮点误差得到44.999后被int截成44。
    dist_x10 = int(round(dist_cm * 10))

    # 一个有符号字节只能表示-128~127，超出时进行限幅。
    if dist_x10 > 127:
        dist_x10 = 127
    elif dist_x10 < -128:
        dist_x10 = -128

    # & 0xFF把负数转换为UART需要发送的8位补码。
    # 正数不会发生变化：45 & 0xFF仍然等于0x2D。
    data_byte = dist_x10 & 0xFF
    frame = ustruct.pack(">BBBB", 0x2C, 0x5A, data_byte, 0xFE)
    uart.write(frame)

    # 打印发送信息
    print(f"[UART] 距离={dist_x10 / 10.0}cm, 有符号数据={dist_x10}, 发送: {' '.join('0x{:02X}'.format(b) for b in frame)}")
# ========================================================
